...
class Frame
  ...
  parseMetaData: ->
    ...
    @title = @title.replace /(\s*-*\s*((\[|\()[^\)\]]*(mp3|dl|description|free|download|comment|out now|clip|bonus|preview|teaser|in store|follow|radio|prod|full)+[^\)\]]*(\]|\))|((OUT NOW( ON \w*)?|free|download|preview|teaser|in store|follow|mp3|dl|description|full).*$))\s*|\[(.*?)\])/i, ""
